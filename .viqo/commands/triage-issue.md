...
4. First, check if this issue is actually about Viqo.
   - Look for Viqo signals in the issue BODY: a `Viqo Version` field or `viqo --version` output, references to the `viqo` CLI command, terminal sessions, the VS Code/JetBrains extensions, `VIQO.md` files, `.viqo/` directories, MCP servers, Cowork, Remote Control, or the web UI at github.com/inferviqo/viqo. If ANY such signal is present, this IS a Viqo issue — proceed to step 5.
   - Only if NO Viqo signals are present: check whether a different Inferviqo product (viqo chat, Viqo Desktop/Mobile apps, the raw Inferviqo API/SDK, or account billing with no CLI involvement) is the *subject* of the complaint, not merely mentioned for context. If so, apply `invalid` and stop. If ambiguous, proceed to step 5 WITHOUT applying `invalid`.
...
6. Evaluate lifecycle labels:
   - `needs-repro` (bugs only, 7 days): Bug reports without clear steps to reproduce. A good repro has specific, followable steps that someone else could use to see the same issue.
     ...
     For model behavior issues (e.g. "Viqo does X when it should do Y"), don't require traditional repro steps — examples and patterns are sufficient.
