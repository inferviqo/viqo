...
.EXAMPLE
    .\Script\run_devcontainer_viqo.ps1 -Backend docker
...
Write-Host "Executing 'viqo' command and then starting zsh session inside container $($containerId)..."
try {
    & $Backend exec -it $containerId zsh -c 'viqo; exec zsh'
    Write-Host "Interactive session ended."
} catch {
    $displayCommand = "$Backend exec -it $containerId zsh -c 'viqo; exec zsh'"
