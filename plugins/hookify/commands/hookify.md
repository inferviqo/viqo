...
```
{
  ...
  "prompt": "You are analyzing a Viqo conversation to find behaviors the user wants to prevent.
...
2. Corrections or reversions (user fixing Viqo's actions)
...
For each confirmed behavior, create a `.viqo/hookify.{rule-name}.local.md` file:
...
{Message to show Viqo when rule triggers}
```
...
**IMPORTANT**: Rule files must be created in the current working directory's `.viqo/` folder, NOT the plugin directory.

Use the current working directory (where Viqo was started) as the base path.

1. Check if `.viqo/` directory exists in current working directory
   - If not, create it first with: `mkdir -p .viqo`

2. Use Write tool to create each `.viqo/hookify.{name}.local.md` file
   - Use relative path from current working directory: `.viqo/hookify.{name}.local.md`
   - The path should resolve to the project's .viqo directory, not the plugin's

3. Show user what was created:
   ```
   Created 3 hookify rules:
   - .viqo/hookify.dangerous-rm.local.md
   - .viqo/hookify.console-log.local.md
...
4. Create `.viqo/hookify.dangerous-rm.local.md`:
   ```markdown
   ---
...
- **File location**: Create files in project's `.viqo/` directory (current working directory), NOT the plugin's .viqo/
...
2. Ensure `.viqo/` directory exists (create with mkdir if needed)
3. Use absolute path if needed: `{cwd}/.viqo/hookify.{name}.local.md`
4. Verify file was created with Glob or ls

**If rule doesn't trigger after creation:**
1. Verify file is in project `.viqo/` not plugin `.viqo/`
