...
Comprehensive guidance on creating Viqo slash commands, including file format, frontmatter options, dynamic arguments, and best practices.
...
- Plugin-specific features (${VIQO_PLUGIN_ROOT}, plugin patterns)
...
- **plugin-features-reference.md**: Plugin-specific command features
  - Plugin command discovery and organization
  - ${VIQO_PLUGIN_ROOT} environment variable usage
  - Plugin command patterns (configuration, template, multi-script)
...
Viqo loads references and examples as needed based on task.
...
- **Project**: `.viqo/commands/` (shared with team)
- **Personal**: `~/.viqo/commands/` (your commands)
...
- ✓ Plugin command patterns (${VIQO_PLUGIN_ROOT}, discovery, organization)
...
6. Test examples work with current Viqo
