...
if [ ! -f ".viqo/command-initialized" ]; then
...
Checking for user config: .viqo/plugin-name.local.md

if [ -f ".viqo/plugin-name.local.md" ]; then
  # Parse YAML frontmatter for settings
  VERBOSE=$(grep "^verbose:" .viqo/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  COLOR=$(grep "^color:" .viqo/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  MAX_RESULTS=$(grep "^max_results:" .viqo/plugin-name.local.md | cut -d: -f2 | tr -d ' ')

  echo "✓ Using user configuration"
else
  echo "Using default configuration"
  echo "Create .viqo/plugin-name.local.md to customize"
fi
...
Or set in .viqo/plugin-name.local.md:
\`\`\`yaml
---
format: yaml
output: custom.txt
...
if [ -f ".viqo/operation-completed.flag" ]; then
  ℹ️  Operation already completed

  Completed at: $(cat .viqo/operation-completed.flag)

  To re-run:
  1. Remove flag: rm .viqo/operation-completed.flag
  2. Run command again
...
Marking complete...
echo "$(date)" > .viqo/operation-completed.flag
```
