...
This reference covers features and patterns specific to commands bundled in Viqo plugins.

## Table of Contents

- [Plugin Command Discovery](#plugin-command-discovery)
- [VIQO_PLUGIN_ROOT Environment Variable](#claude_plugin_root-environment-variable)
...
Viqo automatically discovers commands in plugins using the following locations:
...
## VIQO_PLUGIN_ROOT Environment Variable

### Purpose

`${VIQO_PLUGIN_ROOT}` is a special environment variable available in plugin commands that resolves to the absolute path of the plugin directory.
...
Run analysis: !`node ${VIQO_PLUGIN_ROOT}/scripts/analyze.js`

Read template: @${VIQO_PLUGIN_ROOT}/templates/report.md
```
...
Lint results: !`node ${VIQO_PLUGIN_ROOT}/bin/lint.js $1`
...
Configuration: @${VIQO_PLUGIN_ROOT}/config/deploy-config.json
...
Use this template: @${VIQO_PLUGIN_ROOT}/templates/api-report.md
...
Step 1 - Prepare: !`bash ${VIQO_PLUGIN_ROOT}/scripts/prepare.sh $1`
Step 2 - Config: @${VIQO_PLUGIN_ROOT}/config/$1.json
Step 3 - Execute: !`${VIQO_PLUGIN_ROOT}/bin/execute $1`
...
1. **Always use for plugin-internal paths:**
   ```markdown
   # Good
   @${VIQO_PLUGIN_ROOT}/templates/foo.md
...
2. **Validate file existence:**
   ...
   !`test -f ${VIQO_PLUGIN_ROOT}/config.json && echo "exists" || echo "missing"`

   If config exists, load it: @${VIQO_PLUGIN_ROOT}/config.json
...
4. **Combine with arguments:**
   ```markdown
   Run: !`${VIQO_PLUGIN_ROOT}/bin/process.sh $1 $2`
   ```
...
- Verify syntax is exact: `${VIQO_PLUGIN_ROOT}`
...
Load configuration: @${VIQO_PLUGIN_ROOT}/deploy-config.json
...
Template: @${VIQO_PLUGIN_ROOT}/templates/component-docs.md
...
Validate: !`bash ${VIQO_PLUGIN_ROOT}/scripts/validate.sh`
...
Environment config: @${VIQO_PLUGIN_ROOT}/config/$1.json
...
Cache directory: ${VIQO_PLUGIN_ROOT}/cache/

Analyze @$1 and save results to cache:
!`mkdir -p ${VIQO_PLUGIN_ROOT}/cache && date > ${VIQO_PLUGIN_ROOT}/cache/last-run.txt`
...
- Viqo will automatically use Task tool to launch agent
...
- Mention skill by name to hint Viqo should invoke it
...
- Config exists: !`test -f ${VIQO_PLUGIN_ROOT}/config.json && echo "✓" || echo "✗"`
- Scripts exist: !`test -d ${VIQO_PLUGIN_ROOT}/scripts && echo "✓" || echo "✗"`
- Tools available: !`test -x ${VIQO_PLUGIN_ROOT}/bin/analyze && echo "✓" || echo "✗"`
...
Try processing: !`node ${VIQO_PLUGIN_ROOT}/scripts/process.js $1 2>&1 || echo "ERROR: $?"`
...
1. **Use ${VIQO_PLUGIN_ROOT} for all plugin-internal paths**
