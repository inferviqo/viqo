...
head -n 20 .viqo/commands/my-command.md | grep -A 10 "^---"

# Check for closing frontmatter marker
head -n 20 .viqo/commands/my-command.md | grep -c "^---" # Should be 2

# Verify file has .md extension
ls .viqo/commands/*.md

# Check file is in correct location
test -f .viqo/commands/my-command.md && echo "Found" || echo "Missing"
```
...
cat > .viqo/commands/test-bash.md << 'EOF'
---
...
cat > .viqo/commands/test-forbidden.md << 'EOF'
---
...
# Command: .viqo/commands/risky-operation.md
...
COMMANDS_CHANGED=$(git diff --cached --name-only | grep "\.viqo/commands/.*\.md")
  ...
  test:
    ...
    steps:
      ...
      - name: Validate command structure
        run: |
          for cmd in .viqo/commands/*.md; do
            echo "Testing: $cmd"
      ...
      - name: Check for TODOs
        run: |
          if grep -r "TODO" .viqo/commands/; then
...
```bash
# Monitor Viqo during command execution
# In terminal 1:
viqo --debug

# In terminal 2:
watch -n 1 'ps aux | grep viqo'
...
```bash
# Check file location
ls -la .viqo/commands/my-command.md

# Check permissions
chmod 644 .viqo/commands/my-command.md

# Check syntax
head -n 20 .viqo/commands/my-command.md
...
```bash
# Verify syntax
grep '\$1' .viqo/commands/my-command.md
grep '\$ARGUMENTS' .viqo/commands/my-command.md

# Test with simple command first
echo "Test: \$1 and \$2" > .viqo/commands/test-args.md
```
...
```bash
# Check allowed-tools
grep "allowed-tools" .viqo/commands/my-command.md

# Verify command syntax
grep '!\`' .viqo/commands/my-command.md
