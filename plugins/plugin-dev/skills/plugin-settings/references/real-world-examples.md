...
Detailed analysis of how production plugins use the `.viqo/plugin-name.local.md` pattern.
...
SWARM_STATE_FILE=".viqo/multi-agent-swarm.local.md"
...
```bash
cat > "$WORKTREE_PATH/.viqo/multi-agent-swarm.local.md" <<EOF
---
...
sed "s/^pr_number: .*/pr_number: $PR_NUM/" \
  ".viqo/multi-agent-swarm.local.md" > temp.md
mv temp.md ".viqo/multi-agent-swarm.local.md"
```
...
Document any changes needed in VIQO.md.
```
...
**Purpose:** Prevent session exit and loop Viqo's output back as input
...
# Create state file
cat > ".viqo/ralph-loop.local.md" <<EOF
---
iteration: 1
...
echo "Ralph loop initialized: .viqo/ralph-loop.local.md"
```
...
|---------|-------------------|--------------|
| **File** | `.viqo/multi-agent-swarm.local.md` | `.viqo/ralph-loop.local.md` |
...
```bash
# BAD
FILE="/Users/alice/.viqo/my-plugin.local.md"

# GOOD
FILE=".viqo/my-plugin.local.md"
```
...
The `.viqo/plugin-name.local.md` pattern provides:
